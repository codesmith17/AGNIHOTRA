...
function getAgnihotraTimesIterative(dateUTC, lat, lon, tzHours) {
  ...
  function getSunDetails(timestampUTC) {
    ...
    const tangentSquaredObliquity = Math.pow(
      Math.tan(degreesToRadians * (eps / 2)),
      2
    );
  ...
  let riseUTC =
    dateUTC.getTime() +
    (solarNoon - cosineOfHourAngle / 15 - tzHours) * 3600000;
  ...
  let sunrise =
    12 + tzHours - lon / 15 - sunriseDetails.equationOfTime / 60 - H_rise / 15;
  ...
  let sunset =
    12 + tzHours - lon / 15 - sunsetDetails.equationOfTime / 60 + H_set / 15;
...
dates.forEach((dateStr) => {
  ...
  const diffRise = Math.abs(
    hmsToSec(target.sunrise) - Math.round(res.sunrise * 3600)
  );
  const diffSet = Math.abs(
    hmsToSec(target.sunset) - Math.round(res.sunset * 3600)
  );

  console.log(
    `${dateStr} | Sunrise | ${target.sunrise}     | ${calcRise}   | ${diffRise}`
  );
  console.log(
    `${dateStr} | Sunset  | ${target.sunset}     | ${calcSet}   | ${diffSet}`
  );
});
